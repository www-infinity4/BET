(function(){
  ...
  const catalog = Array.isArray(window.BET_CATALOG) ? window.BET_CATALOG : [];
  ...
  function renderPremium(){
    const targets=Array.isArray(window.BET_PREMIUM_TARGETS)?window.BET_PREMIUM_TARGETS:[];
  ...
  function loadProgram(block,elapsed){
    ...
    if(sourceEnded){
      showCard("BET",program.title,"The complete source ended. Next scheduled program starts soon.");
      return;
    }
  ...
  function localShareCredit(reference){
    ...
    profile.ledger.push({id:`tx-${attemptId}`,type:awarded?"share_reward":"share_credit",amount:awarded,balance:profile.tokens,pendingShareCredits:profile.pendingShareCredits,reason:awarded?"Share reward: 10 completed shares":`Confirmed BET share ${profile.pendingShareCredits}/10`,referenceId:attemptId,createdAt:Date.now()});
  ...
  async function share(){
    const block=currentBlock(Date.now()), title=block?block.program.title:"BET";
    const payload={title:`${title} · BET`,text:`Watch ${title} on BET.`,url:location.href};
